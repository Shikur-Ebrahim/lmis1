...
import { LogOut, Edit3, Save, EyeOff, Star, Award, Activity, TrendingUp, Mail, Phone, Shield, Ban, Copy, FileText, Check, Building2, Globe, Zap, CreditCard } from "lucide-react"
    ...
    const unsubscribeUser = onSnapshot(
      ...
      (docSnapshot) => {
        ...
        if (docSnapshot.exists()) {
          ...
          // Auto-fix for missing application number
          if (!userData.applicationNumber && userData.id) {
            const newAppNumber = `APP-${Date.now()}`
            console.log("[Auto-Fix] Missing applicationNumber. Generating:", newAppNumber)
            updateDoc(doc(db, "users", userData.id), {
              applicationNumber: newAppNumber
            }).catch(err => console.error("[Auto-Fix] Failed to save applicationNumber:", err))
          }
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                {userData.applicationNumber && (
                  <p className="text-[10px] sm:text-xs font-mono text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full mt-1 flex items-center w-fit border border-blue-100 font-bold">
                    {userData.applicationNumber}
                  </p>
                )}
              </div>
          ...
          <div className="space-y-8">
            ...
            >
              <div className="relative z-10">
                <div className="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-6">
                  <div className="p-4 bg-white/20 rounded-full animate-bounce shadow-lg shadow-white/10">
                  ...
                  <div className="flex-1 text-center md:text-left">
                    ...
                    <p className="text-xl text-white/90 mb-0 drop-shadow-md">
                      {applicationStatus === "Approved"
            ...
            <div className="bg-white rounded-2xl shadow-xl border border-gray-200 p-8 hover:shadow-2xl transition-all duration-500">
              <div className="text-center">
                ...
                {userData.applicationNumber && (
                  <div className="flex items-center justify-center mt-3 space-x-2 bg-gray-50 py-2 px-4 rounded-xl border border-gray-100 w-fit mx-auto cursor-pointer hover:bg-gray-100 transition-colors" onClick={copyApplicationNumber} title="Click to copy">
                    <FileText className="w-5 h-5 text-blue-500" />
                    <p className="text-gray-600 font-mono font-bold text-sm">{userData.applicationNumber}</p>
                  ...
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-blue-700 mt-1">
                        ...
                        <br />
                        <strong>Application ID:</strong> {userData?.applicationNumber || "N/A"}
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="md:col-span-2">
                      ...
                      <div className="flex gap-2">
                        <input
                          type="text"
                          value={userData.applicationNumber || "Generating..."}
                          readOnly
                          className="w-full p-4 border border-gray-300 rounded-xl bg-blue-50/50 font-mono text-blue-700 font-bold"
                        />
                        <button
                          onClick={copyApplicationNumber}
                          className="px-4 py-2 bg-blue-100 text-blue-600 rounded-xl hover:bg-blue-200 transition-all flex items-center justify-center shrink-0"
                          title="Copy Application Number"
                        >
                          {copiedApplicationNo ? <CheckCircle className="w-5 h-5" /> : <Copy className="w-5 h-5" />}
                        </button>
                      </div>
                    </div>
                  </div>
